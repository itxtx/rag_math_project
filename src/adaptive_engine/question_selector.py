...
class QuestionSelector:
    ...
    async def _determine_difficulty(self, learner_id: str, concept_id: Optional[str]) -> str:
        ...
        if concept_id:
            ...
            if knowledge:
                ...
                if attempts == 0: difficulty = "beginner"
                elif score < LOW_SCORE_THRESHOLD: difficulty = "beginner" 
                elif score >= PERFECT_SCORE_THRESHOLD: difficulty = "advanced" 
    ...
    async def select_next_question(self, learner_id: str, target_doc_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if not selected_concept_block_info:
            print("QuestionSelector: Could not select any suitable concept (review or new)" + (f" for topic '{target_doc_id}'." if target_doc_id else "."))
            return None
        ...
        if not context_chunks_data:
            print(f"QuestionSelector: No context chunks found by retriever for parent_block_id '{parent_block_id_for_qg}'.")
            return None
