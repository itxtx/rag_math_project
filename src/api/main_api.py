...
from typing import Dict, Any, Optional, List as PyList # Renamed to avoid conflict with Pydantic List
...
@app.get("/api/v1/topics", 
           response_model=PyList[TopicResponse], # Use PyList from typing
...
async def list_available_topics(
    ...
    try:
        ...
        if not topics:
            # If curriculum map is empty, try to reload it once.
            print("API /topics: Curriculum map was empty, attempting reload.")
            components.question_selector._load_curriculum_map() # Call the sync method
            topics = components.question_selector.get_available_topics()
...
@app.post("/api/v1/interaction/start", 
            ...
            description="Retrieves the next question for a given learner. Optionally, a specific topic_id (doc_id) can be provided to focus questions on that topic.",
            responses={
...
async def start_learner_interaction(
    ...
    topic_id: Optional[str] = Query(None, description="Optional topic_id (doc_id) to focus question selection."),
    components: RAGSystemComponents = Depends(get_rag_components)
):
    print(f"API: Received request to start interaction for learner_id: {request.learner_id}, topic_id: {topic_id}")
    try:
        next_question_info = await components.question_selector.select_next_question(
            learner_id=request.learner_id,
            target_doc_id=topic_id # Pass the topic_id here
        )
        if not next_question_info:
            detail_msg = "Could not select a next question."
            if topic_id:
                detail_msg += f" No suitable questions found for topic '{topic_id}' or learner has mastered it."
            else:
...
if __name__ == "__main__":
    import uvicorn
    import datetime # For health check timestamp
